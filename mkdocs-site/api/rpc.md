...
!!! warning "Testnet only"
    This endpoint is disabled in mainnet mode (returns HTTP 410 GONE). On mainnet, generate keys client-side using an [SDK](sdks.md).
...
!!! note "Faucet limits"
    Maximum 100 UDAG per request. Testnet only (disabled in mainnet mode).
...
!!! warning "Testnet only"
    The `/tx` endpoint accepts private keys in the request body. This is disabled in mainnet mode. Use `/tx/submit` with pre-signed transactions for mainnet.
...
See [Transaction Format](transactions.md) for the signing specification.
...
- [Transaction Format](transactions.md) — signing specification for `/tx/submit`
- [SDKs](sdks.md) — client libraries for all transaction types
- [Monitoring](../operations/monitoring.md) — using `/metrics` with Prometheus/Grafana
