...
- **Bug #239 (MEDIUM): Orphan buffer accepts non-zero coinbase vertices** — Vertices with invalid coinbase would be buffered as orphans, fail at `try_insert()`, and waste buffer space. Fix: coinbase check added to `insert_orphan()`.
- **Checkpoint co-signing** (Issue #3 from review): Verified already correct — co-signers only validate at exact `checkpoint.round == state_fin`, silently skip if state advanced. `save_checkpoint_state()` ensures GetCheckpoint serves the correct state.
- **Bridge release vote-time set** (Issue #5 from review): Acknowledged as design tradeoff — threshold recalculated per vote using current active set. Epoch transitions between votes can shift the threshold. Documenting as known behavior; fix would require snapshotting active set at first vote (similar to governance quorum snapshot).
...
- **Known limitations (architecture decisions, not bugs):**
  ...
  - **No EIP-1559 fee market** — current model: MIN_FEE + mempool fee-based eviction. Fee estimation endpoint added. Base fee and priority fees are a future enhancement.
  - **No in-place key rotation** — validator keys are permanent. Compromised key requires unstake/restake cycle (2,016 round cooldown). Council keys: Remove+Add proposal. A single compromised key = 1/21 votes (4.7%); needs 14/21 to pass anything.
- **Addressed concerns (already protected):**
  - **Governance timelock self-reference**: `execute_at_round` is computed at Active→PassedPending transition and is IMMUTABLE. Reducing `execution_delay_rounds` via governance only affects FUTURE proposals. Sequential attack still works: proposal A reduces delay to 2016, executes after old delay, then proposal B uses the new 2016-round delay (~2.8 hours). This is a defensible design choice — 2.8 hours provides a monitoring/response window. Documented in engine.rs lines 1864-1872.
  - **Vertex/block size limit**: `MAX_VERTEX_BYTES = 1MB` enforced in `try_insert()`. `TooLarge` error variant. Prevents DoS via oversized vertices.
  - **Peer banning**: Exponential backoff (2^n seconds, max 1 hour) for rate limit violators, garbage handshakes, wrong protocol version. Banned at TCP accept before Noise handshake.
