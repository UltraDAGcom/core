...
const S = {
  ...
  label: { fontSize: 10.5, fontWeight: 600, color: 'rgba(255,255,255,0.32)', letterSpacing: 1.2, textTransform: 'uppercase' as const, marginBottom: 6 },
    ...
    <div style={S.card}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 13.5, fontWeight: 600, color: 'rgba(255,255,255,0.55)' }}>{title}</span>
        </div>
        {action}
      </div>
      {children}
...
export function SmartAccountPage({ walletAddress, nodeUrl }: { walletAddress?: string; nodeUrl: string }) {
  ...
  if (!walletAddress) return <div style={{ padding: '18px 26px', color: 'rgba(255,255,255,0.3)', fontSize: 13, fontFamily: "'DM Sans',sans-serif" }}>Create a wallet first.</div>;
  if (loading) return <div style={{ padding: '18px 26px', color: 'rgba(255,255,255,0.3)', fontSize: 13, fontFamily: "'DM Sans',sans-serif", animation: 'pulse 1.5s infinite' }}>Loading SmartAccount...</div>;
  ...
  return (
    <div style={{ padding: '18px 26px', fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22, animation: 'slideUp 0.3s ease' }}>
        <div>
          <h1 style={{ fontSize: 21, fontWeight: 700, color: '#fff' }}>SmartAccount</h1>
          <p style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.25)', marginTop: 2 }}>Keys, recovery, spending limits, and name</p>
        </div>
        ...
        <Section icon="◎" color="#00E0C4" title="Your Name"
          ...
          {nameInfo ? (
            <div>
              ...
              {nameInfo.expiry_round && <p style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.22)' }}>Expires round {nameInfo.expiry_round.toLocaleString()}</p>}
            </div>
          ) : localName ? (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              ...
              <p style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.22)' }}>Saved locally. On-chain registration available via SmartOp.</p>
            </div>
          ) : showNameForm ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ position: 'relative' }}>
                ...
                {nameChecking && <span style={{ position: 'absolute', right: 12, top: 11, color: 'rgba(255,255,255,0.2)', fontSize: 11 }}>...</span>}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => setShowNameForm(false)} style={S.btn('rgba(255,255,255,0.3)')}>Cancel</button>
              </div>
            </div>
          ) : (
            <p style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.22)' }}>No name registered. Claim one like <span style={{ color: '#00E0C4' }}>alice</span> or <span style={{ color: '#00E0C4' }}>john29</span>.</p>
          )}
        </Section>
          ...
          {info && info.authorized_keys.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {info.authorized_keys.map(key => (
                <div key={key.key_id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.025)', borderRadius: 10, padding: '10px 13px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 600, color: '#fff' }}>{key.label}</div>
                      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.22)', ...S.mono }}>{key.key_id.slice(0, 12)}… ({key.key_type === 'p256' ? 'Passkey' : 'Ed25519'})</div>
                    </div>
                  </div>
                  <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)' }}>{key.daily_limit ? `${fmt(key.daily_limit)}/day` : 'No limit'}</div>
                </div>
              ))}
              ...
              <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.18)' }}>Add a backup device by scanning a QR from another wallet.</p>
            </div>
          ) : (
            <p style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.22)' }}>{info ? 'Key auto-registers on first transaction.' : 'SmartAccount activates when you send or receive.'}</p>
          )}
        </Section>
        ...
        <Section icon="♛" color="#A855F7" title="Social Recovery"
          ...
          {info?.has_recovery ? (
            <div>
              ...
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>{info.recovery_threshold}-of-{info.guardian_count} guardians required to recover</p>
              <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.18)', marginTop: 4 }}>If you lose all devices, contact your guardians. Each one opens their wallet and approves your recovery. After {info.recovery_threshold} approve, a time-lock starts — then your new device gets access.</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ background: 'rgba(168,85,247,0.04)', border: '1px solid rgba(168,85,247,0.1)', borderRadius: 10, padding: '12px 14px' }}>
                ...
                <ol style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.3)', lineHeight: 1.7, margin: 0, paddingLeft: 16 }}>
                  <li>Pick friends or family who have UltraDAG wallets</li>
                  <li>If you lose all devices, call them</li>
                  <li>Each guardian opens <strong style={{ color: 'rgba(255,255,255,0.5)' }}>their own wallet</strong> and taps "Approve Recovery"</li>
                ...
                <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.2)', marginTop: 8 }}>Guardians can never access your funds — they can only authorize a key change.</p>
              </div>

              {/* Guardian inputs */}
              <div style={{ fontSize: 10.5, fontWeight: 600, color: 'rgba(255,255,255,0.32)', letterSpacing: 1, marginTop: 4 }}>GUARDIAN WALLET ADDRESSES</div>
              {guardians.map((g, i) => (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.25)' }}>How many must approve:</span>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => setShowRecoveryForm(false)} style={S.btn('rgba(255,255,255,0.3)')}>Cancel</button>
              </div>
            </div>
          ) : (
            <div>
              <p style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.22)' }}>No recovery guardians set.</p>
              <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.15)', marginTop: 4, lineHeight: 1.6 }}>
          ...
          {info?.has_policy ? (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {[
                ...
                <div key={i} style={S.stat}>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', letterSpacing: 1, marginBottom: 3 }}>{x.l}</div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: '#fff' }}>{x.v}</div>
                </div>
              ))}
            </div>
          ) : showPolicyForm ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => setShowPolicyForm(false)} style={S.btn('rgba(255,255,255,0.3)')}>Cancel</button>
              </div>
            </div>
          ) : (
            <div>
              <p style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.22)' }}>No spending limits set.</p>
              <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.15)', marginTop: 4 }}>Protect against key theft with daily limits and vault delays.</p>
            </div>
          )}
        </Section>
      </div>
