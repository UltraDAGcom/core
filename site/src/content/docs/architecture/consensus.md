...
description: "Technical deep dive into UltraDAG's DAG-based Byzantine Fault Tolerant consensus — vertex production, finality, ordering, and pruning."
order: 2
...
<div class="callout callout-note"><div class="callout-title">Hash computation</div>The vertex hash is <strong>not stored</strong> — it is computed via <code>DagVertex::hash()</code> which delegates to <code>block.hash()</code> (the block header hash). The <code>signable_bytes()</code> method (used for Ed25519 signing) is different: <code>NETWORK_ID || b"vertex" || block.hash() || parent_hashes || round || validator</code>. The hash and signable_bytes are distinct computations.</div>
...
<div class="callout callout-info"><div class="callout-title">Why Blake3 scoring?</div>Deterministic parent scoring ensures high overlap between validators' parent selections, which accelerates finality convergence. Random or arbitrary parent selection would lead to slower finality as each vertex would cover fewer unique ancestors.</div>
...
<div class="callout callout-danger"><div class="callout-title">Slashing is deterministic</div>All honest validators independently detect and apply the same slash, producing identical state transitions. There is no voting or subjective judgment — equivocation evidence is cryptographically verifiable.</div>
...
<div class="callout callout-tip"><div class="callout-title">Archive mode</div>Run with <code>--archive</code> to disable pruning entirely. Archive nodes retain the full DAG history and can serve historical queries. This requires more storage but is useful for block explorers and analytics.</div>
...
- [P2P Network](/docs/architecture/network) — how vertices are propagated
- [State Engine](/docs/architecture/state-engine) — how finalized vertices become account state
- [Formal Verification](/docs/technical/formal-verification) — TLA+ proof of safety properties
