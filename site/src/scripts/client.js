  ...
  const examples = [
    {
      ...
      code: '// Fetch network status from testnet\n'
        ...
        + 'console.log("Validators:", status.validator_count || status.active_stakers || 0);\n'
        + 'console.log("Peers:", status.peer_count || status.connected_peers || 0);\n'
    ...
    },
    {
      ...
      code: '// Look up an account balance (UltraDAG addresses are 20 bytes = 40 hex chars)\n'
    ...
    },
    {
      ...
      code: '// List stake-based validators and their commissions.\n'
        + '// NOTE: the current testnet runs in allowlist mode (see --validator-key),\n'
        + '// so /validators will report zero stake-based validators even though\n'
        + '// /status shows 5 active consensus nodes. Use /status.validator_count\n'
        + '// for the authoritative count.\n'
        + 'const res = await fetch("' + NODE + '/validators");\n'
        + 'const data = await res.json();\n'
        + 'const validators = data.validators || [];\n'
        + 'const status = await (await fetch("' + NODE + '/status")).json();\n\n'
        + 'console.log("Stake-based validators:", validators.length);\n'
        + 'console.log("Consensus validators (from /status):", status.validator_count || 0);\n'
        + 'console.log("---");\n\n'
        + 'if (validators.length === 0) {\n'
        + '  console.log("(allowlist testnet — no stake-based validators yet)");\n'
        + '} else {\n'
