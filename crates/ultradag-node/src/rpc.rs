            ...
            match state.resolve_name(name) {
                Some(addr) => {
                    ...
                    json_response(StatusCode::OK, &serde_json::json!({
                        ...
                        // Pockets are on the SmartAccount, not the name profile.
                        // Derive addresses for each registered pocket label.
                        "pockets": state.smart_account(&addr).map(|cfg| {
                            cfg.pockets.iter().map(|label| {
                                let pocket_addr = ultradag_coin::tx::name_registry::derive_pocket_address(&addr, label);
                                serde_json::json!({
                                    "label": label,
                                    "address": pocket_addr.to_hex(),
