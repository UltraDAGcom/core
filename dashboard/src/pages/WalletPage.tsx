    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(6px)' }}>
      <div style={{ ...S.card, maxWidth: 440, width: '100%', boxShadow: '0 24px 60px rgba(0,0,0,0.6)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
          <h2 style={{ fontSize: 15, fontWeight: 700, color: 'var(--dag-text)' }}>{title}</h2>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'var(--dag-subheading)', cursor: 'pointer', fontSize: 16 }}>✕</button>
        </div>
        {children}
      </div>
    </div>
    ...
    <Modal open={open} title="Change Password" onClose={close}>
      <form onSubmit={async e => { e.preventDefault(); setErr(''); setOk(false); if (np.length < 8) { setErr('Min 8 chars.'); return; } if (np !== cp) { setErr("Don't match."); return; } setLd(true); try { (await changePassword(cur, np)) ? setOk(true) : setErr('Wrong current password.'); } catch { setErr('Failed.'); } finally { setLd(false); } }}
        ...
        {[{ l: 'Current', v: cur, s: setCur }, { l: 'New', v: np, s: setNp }, { l: 'Confirm', v: cp, s: setCp }].map((f, i) => (
          <div key={i}><div style={{ fontSize: 10, color: 'var(--dag-text-muted)', marginBottom: 3 }}>{f.l}</div><input type="password" style={S.input} value={f.v} onChange={e => f.s(e.target.value)} required /></div>
        ))}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 4 }}>
          <button type="button" onClick={close} style={S.btn('var(--dag-text-muted)')}>Cancel</button>
  ...
  if (!unlocked) {
    return (
      <div style={{ padding: '18px 26px', fontFamily: "'DM Sans',sans-serif" }}>
        ...
        <h1 style={{ fontSize: 21, fontWeight: 700, color: 'var(--dag-text)', marginBottom: 4, animation: 'slideUp 0.3s ease' }}>Wallet</h1>
        <p style={{ fontSize: 11.5, color: 'var(--dag-subheading)' }}>Manage your UltraDAG wallets</p>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '70px 0', gap: 18 }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--dag-text)' }}>{hasStore ? 'Wallet Locked' : 'No Wallet Yet'}</div>
            <div style={{ fontSize: 11.5, color: 'var(--dag-subheading)', marginTop: 4, maxWidth: 280 }}>{hasStore ? 'Enter your password to access your wallets.' : 'Create a new wallet or import an existing one.'}</div>
          </div>
    ...
    <div style={{ padding: m ? '12px 14px' : '18px 26px', fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: m ? 'flex-start' : 'center', marginBottom: 18, animation: 'slideUp 0.3s ease', flexDirection: m ? 'column' : 'row', gap: m ? 10 : 0 }}>
        ...
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setShowPwModal(true)} style={S.btn('var(--dag-text-muted)')}>⚿ Password</button>
          <button onClick={handleExport} style={S.btn('var(--dag-text-muted)')}>↓ Export</button>
      ...
      {wallets.length === 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '50px 0', gap: 14 }}>
          ...
          <p style={{ fontSize: 12, color: 'var(--dag-subheading)' }}>No wallets yet. Add one to get started.</p>
          <button onClick={() => setShowAddModal(true)} style={S.btnSolid}>Add Wallet</button>
        </div>
      ) : (
            ...
            {wallets.slice((walletPage - 1) * WALLET_PAGE_SIZE, walletPage * WALLET_PAGE_SIZE).map((w, _pi) => {
              ...
              return (
                <div key={w.address} onClick={() => setSel(active ? null : i)} style={{
                  ...
                  borderColor: active ? 'rgba(0,224,196,0.25)' : 'var(--dag-border)',
                  background: active ? 'rgba(0,224,196,0.025)' : 'var(--dag-card)',
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{
                        ...
                        fontSize: 15, fontWeight: 800, color: 'var(--dag-text)',
                        boxShadow: active ? '0 0 12px rgba(0,224,196,0.15)' : 'none',
                      }}>{w.name[0]?.toUpperCase() || '?'}</div>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--dag-text)' }}>{w.name}</span>
                        ...
                        <div style={{ fontSize: 10, color: 'var(--dag-text-faint)', ...S.mono, marginTop: 2 }}>{shortAddr(w.address)}</div>
                      </div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--dag-text)', ...S.mono }}>{fmt(totalVal)}</div>
                      <div style={{ fontSize: 9, color: 'var(--dag-text-faint)' }}>UDAG</div>
                    </div>
                  </div>
                  {/* Mini balance bar */}
                  {totalVal > 0 && (
                    <div style={{ display: 'flex', borderRadius: 3, overflow: 'hidden', height: 3, marginTop: 10, background: 'var(--dag-card)' }}>
          ...
          <div style={S.card}>
            {selected ? (
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
                  <div style={{
                    ...
                    background: pw?.address === selected.address ? 'linear-gradient(135deg,#00E0C4,#0066FF)' : 'var(--dag-card-hover)',
                    fontSize: 20, fontWeight: 800, color: 'var(--dag-text)',
                  ...
                  {[
                    ...
                    <div key={i} style={S.stat}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 3 }}>
                        ...
                        <span style={{ fontSize: 8.5, color: 'var(--dag-text-faint)', letterSpacing: 1 }}>{x.l}</span>
                      </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 260, gap: 10 }}>
                ...
                <p style={{ fontSize: 12, color: 'var(--dag-text-faint)' }}>Select a wallet to view details</p>
                <p style={{ fontSize: 10, color: 'var(--dag-text-faint)' }}>Click any wallet on the left</p>
              </div>
            )}
          </div>
        </div>
